...
cut_initial = 5 * sampling_rate  # Cut the first 30 seconds
cut_final = 5 * sampling_rate  # Cut the last 5 seconds

# Paths
base_path = os.path.dirname(__file__)
eeg_data_folder = os.path.join(base_path, "eeg-data/EthanNewData")
...
for video_index in range(1, total_videos + 1):
    eeg_file_path = os.path.join(
        eeg_data_folder, f"OpenBCI-RAW-2024-0514_vid{video_index}.txt"
    )
...
# Save the array
print(X.shape)
np.save(os.path.join(base_path, "preprocessed_data_Ethan.npy"), X)
